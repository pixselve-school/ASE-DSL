...
export class DslLibraryFileSystemProvider implements vscode.FileSystemProvider {
  static register(context: vscode.ExtensionContext) {
    context.subscriptions.push(
      vscode.workspace.registerFileSystemProvider(
        "builtin",
        // @ts-ignore
        new DslLibraryFileSystemProvider(context),
        {
          isReadonly: true,
  ...
  stat(uri: vscode.Uri): vscode.FileStat {
    ...
    return {
      ctime: date,
      mtime: date,
      size: Buffer.from(builtInMiniMechaCode).length,
  ...
  readFile(uri: vscode.Uri): Uint8Array {
    ...
    return new Uint8Array(Buffer.from(builtInMiniMechaCode));
  }
...
function startLanguageClient(context: vscode.ExtensionContext): LanguageClient {
  ...
  const fileSystemWatcher =
    vscode.workspace.createFileSystemWatcher("**/*.mini");
  context.subscriptions.push(fileSystemWatcher);

  // Options to control the language client
  const clientOptions: LanguageClientOptions = {
    documentSelector: [{ scheme: "file", language: "mini-mecha-code" }],
    synchronize: {
