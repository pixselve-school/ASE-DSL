...
export class MiniMechaCodeValidator {
    ...
    checkDefVariableIsNotAlreadyDefined(defVariable: DefVariable, accept: ValidationAcceptor): void {
            ...
            for (let statement of loop.statements) {
                ...
                if (isDefVariable(statement)) {
                    ...
                    if (defVar.name === defVariable.name) {
                        accept("error", `Cannot redeclare block-scoped variable ${defVariable.name}.`, {
        ...
        if (isDefFunction(parent)) {
            ...
            if (functionDef.parameters.find(param => param.name === defVariable.name)) {
